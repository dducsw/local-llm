...
router = APIRouter(prefix="/api/slurm", tags=["Slurm & Compute"])
...
async def get_slurm_jobs():
    ...
    code, stdout, _ = await run_slurm_cli_async(cmd)
...
async def submit_slurm_job(body: SubmitJobRequest):
    ...
    if HPC_SSH_HOST:
        code, stdout, stderr = await run_slurm_cli_async(submit_cmd, timeout=15.0, work_dir=HPC_REMOTE_DIR, use_cache=False)
...
async def get_slurm_job_log(job_id: str):
    ...
    # Slurm knows the actual output path, which is more reliable than guessing
    # from the job name or the directory where the gateway was started.
